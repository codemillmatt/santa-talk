...
{
    public static class SendPictureSanta
    {
        static FaceClient faceClient;

        static SendPictureSanta()
        {
            var keys = new ApiKeyServiceClientCredentials(Environment.GetEnvironmentVariable("FaceAPIKey"));

            faceClient = new FaceClient(keys) { Endpoint = Environment.GetEnvironmentVariable("APIEndpoint") };
        }

        [FunctionName("SendPictureSanta")]
            ...
            ILogger log)
        {
            try
            {
                ...
                if (faceClient == null)
                {
                    log.LogInformation("faceClient is null");
                    return new BadRequestObjectResult("faceClient is null");
                }
            ...
            catch (APIErrorException f)
            {
                ...
                return new BadRequestObjectResult("APIErrorException:" + f.Message.ToString());
            ...
            catch (Exception e)
            {
                ...
                return new BadRequestObjectResult("APIErrorException:" + e.Message.ToString());
